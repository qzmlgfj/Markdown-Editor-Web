...
- 图片、Mermaid、任意 HTML、行内公式 `$...$` 暂不渲染。图片保留为 `[图片：alt]` 文本并提示；HTML 和行内公式保留原文，Mermaid 围栏按普通代码输出。
...
已通过 Lint、29 项单元测试和生产构建。Chrome 中验证了子目录部署、英文首次导出、中文行内代码、列表内块、列表起始编号、错误公式反馈、图片占位文本、160 行代码分页及超长代码行。
